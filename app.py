...
# Intentar inicializar GEE al inicio
...
<p class="hero-subtitle">Potenciado con Google Earth Engine, NASA POWER y SRTM para agricultura de precisión</p>
</div>
</div>
""", unsafe_allow_html=True)

# ===== CONFIGURACIÓN DE SATÉLITES DISPONIBLES (ACTUALIZADO CON GEE) =====
...
def convert_to_ee_geometry(gdf):
    """Convierte un GeoDataFrame a geometría de Earth Engine"""
    try:
        ...
        if geometry.geom_type == 'Polygon':
            ...
            # Convertir a formato Earth Engine [[lon, lat], ...]
            ...
            return ee.Geometry.Rectangle([bounds[0], bounds[1], bounds[2], bounds[3]])
    except Exception as e:
        st.error(f"Error convirtiendo geometría a EE: {str(e)}")
        return None
...
def get_sentinel2_ndvi(gdf, fecha_inicio, fecha_fin):
    ...
    try:
        ...
        # Convertir geometría
        roi = convert_to_ee_geometry(gdf)
        if roi is None:
            return None
        ...
        if count == 0:
            st.warning("No se encontraron imágenes Sentinel-2 sin nubes en el período seleccionado")
            return None
        
        # Calcular mediana compuesta
        image = collection.median()
        
        # Calcular NDVI: (B8 - B4) / (B8 + B4)
        ndvi = image.normalizedDifference(['B8', 'B4']).rename('NDVI')
        
        # Recortar a la región de interés
        ...
        image_id = image_info.get('id', 'S2_unknown') if image_info else 'S2_unknown'
        
        return {
            'indice': 'NDVI',
            'valor_promedio': float(ndvi_mean),
        ...
        st.error(f"❌ Error obteniendo Sentinel-2 de GEE: {str(e)}")
        return None

def get_landsat8_ndvi(gdf, fecha_inicio, fecha_fin):
    """Obtiene NDVI de Landsat 8 desde Google Earth Engine"""
    try:
        if not gee_initialized:
            return None
        ...
        count = collection.size().getInfo()
        if count == 0:
            st.warning("No se encontraron imágenes Landsat-8 sin nubes en el período seleccionado")
            return None
        
        # Calcular mediana compuesta
        image = collection.median()
        
        # Calcular NDVI: (B5 - B4) / (B5 + B4)
        ...
        # Obtener información de la imagen
        image_info = collection.first().getInfo()
        image_id = image_info.get('id', 'LC08_unknown') if image_info else 'LC08_unknown'
        
        return {
            'indice': 'NDVI',
            'valor_promedio': float(ndvi_mean),
            'fuente': 'Landsat-8 (Google Earth Engine)',
            'fecha': fecha_fin.strftime('%Y-%m-%d'),
            'id_escena': image_id,
            'cobertura_nubes': 'N/A',
            'resolucion': '30m',
        ...
        st.error(f"❌ Error obteniendo Landsat-8 de GEE: {str(e)}")
        return None

def get_srtm_elevation(gdf):
    """Obtiene datos de elevación SRTM desde Google Earth Engine"""
    try:
        if not gee_initialized:
            return None
        ...
        elevation_stats = dem_clipped.reduceRegion(
            reducer=ee.Reducer.mean(),
            geometry=roi,
            scale=30,
            maxPixels=1e9
        ...
        elev_mean = elevation_stats.get('elevation').getInfo()
        slope_mean = slope_stats.get('slope').getInfo()
        
        return {
            'elevacion_promedio': float(elev_mean) if elev_mean else 0,
            'pendiente_promedio': float(slope_mean) if slope_mean else 0,
            'dem_image': dem_clipped,
            'slope_image': slope,
            'roi': roi
        }
        
    except Exception as e:
        st.error(f"❌ Error obteniendo datos SRTM de GEE: {str(e)}")
        return None

def create_gee_map_layer(image, roi, params, layer_name):
    """Crea una capa de mapa para visualización de GEE"""
    try:
        # Generar URL del tile de GEE
        ...
        st.error(f"Error creando capa de mapa GEE: {str(e)}")
        return None
...
nutriente = None
satelite_seleccionado = "SENTINEL-2_GEE" if gee_initialized else "DATOS_SIMULADOS"
...
with st.sidebar:
    ...
    # Mostrar opciones según disponibilidad de GEE
    if gee_initialized:
        satelite_opciones = ["SENTINEL-2_GEE", "LANDSAT-8_GEE", "DATOS_SIMULADOS"]
        satelite_help = "Selecciona la fuente de datos satelitales reales o simuladas"
    else:
        satelite_opciones = ["DATOS_SIMULADOS"]
        satelite_help = "GEE no disponible. Usando datos simulados."
        st.warning("⚠️ Google Earth Engine no disponible")
    
    satelite_seleccionado = st.selectbox(
        "Satélite:",
        satelite_opciones,
        help=satelite_help,
        index=0 if gee_initialized else 0
    )
    ...
    st.subheader("🏔️ Configuración Topografía")
    usar_srtm_real = st.checkbox("Usar datos SRTM reales de NASA", value=gee_initialized)
...
# ===== FUNCIONES PARA DATOS SATELITALES (ACTUALIZADAS CON GEE) =====
def descargar_datos_sentinel2_gee(gdf, fecha_inicio, fecha_fin, indice='NDVI'):
    """Descarga datos reales de Sentinel-2 desde Google Earth Engine"""
    if not gee_initialized:
        st.warning("⚠️ GEE no inicializado. Usando datos simulados.")
        return generar_datos_simulados(gdf, "TRIGO", indice)
    
    with st.spinner("📡 Descargando datos Sentinel-2 desde Google Earth Engine..."):
        datos_reales = get_sentinel2_ndvi(gdf, fecha_inicio, fecha_fin)
        
        if datos_reales:
            st.success(f"✅ Datos Sentinel-2 descargados: NDVI = {datos_reales['valor_promedio']:.3f}")
            return datos_reales
        else:
            st.warning("⚠️ No se pudieron obtener datos reales. Usando datos simulados.")
            return generar_datos_simulados(gdf, "TRIGO", indice)

def descargar_datos_landsat8_gee(gdf, fecha_inicio, fecha_fin, indice='NDVI'):
    """Descarga datos reales de Landsat-8 desde Google Earth Engine"""
    if not gee_initialized:
        st.warning("⚠️ GEE no inicializado. Usando datos simulados.")
        return generar_datos_simulados(gdf, "TRIGO", indice)
    
    with st.spinner("📡 Descargando datos Landsat-8 desde Google Earth Engine..."):
        datos_reales = get_landsat8_ndvi(gdf, fecha_inicio, fecha_fin)
        
        if datos_reales:
            st.success(f"✅ Datos Landsat-8 descargados: NDVI = {datos_reales['valor_promedio']:.3f}")
            return datos_reales
        else:
            st.warning("⚠️ No se pudieron obtener datos reales. Usando datos simulados.")
            return generar_datos_simulados(gdf, "TRIGO", indice)
...
def descargar_datos_sentinel2_simulado(gdf, fecha_inicio, fecha_fin, indice='NDVI'):
    """Versión simulada para cuando GEE no está disponible"""
    try:
        datos_simulados = {
            'indice': indice,
...
def descargar_datos_landsat8_simulado(gdf, fecha_inicio, fecha_fin, indice='NDVI'):
    """Versión simulada para cuando GEE no está disponible"""
    try:
        datos_simulados = {
            'indice': indice,
...
def generar_dem_srtm_real(gdf):
    """Genera DEM real usando datos SRTM de NASA"""
    try:
        if not gee_initialized:
            st.warning("⚠️ GEE no disponible para SRTM. Usando DEM sintético.")
            return generar_dem_sintetico(gdf, 30.0)
        
        with st.spinner("🗺️ Descargando datos SRTM de NASA..."):
            srtm_data = get_srtm_elevation(gdf)
            
            if srtm_data and 'dem_image' in srtm_data:
                # Obtener bounds
                bounds = gdf.total_bounds
                minx, miny, maxx, maxy = bounds
                ...
                # Para DEM real, necesitaríamos descargar los datos pixel por pixel
                # Por simplicidad, usamos el valor promedio y generamos variación
                elev_prom = srtm_data['elevacion_promedio']
                slope_prom = srtm_data['pendiente_promedio']
                
                # Generar terreno con la elevación promedio
                centroid = gdf.geometry.unary_union.centroid
                seed_value = int(centroid.x * 10000 + centroid.y * 10000) % (2**32)
                rng = np.random.RandomState(seed_value)
                
                # Elevación base del SRTM
                elevacion_base = elev_prom
                
                # Pendiente general basada en SRTM
                slope_x = np.tan(np.radians(slope_prom)) * 0.001
                slope_y = np.tan(np.radians(slope_prom)) * 0.001
                
                # Relieve más suave para datos reales
                relief = np.zeros_like(X)
                n_hills = rng.randint(2, 4)
                for _ in range(n_hills):
                    hill_center_x = rng.uniform(minx, maxx)
                    hill_center_y = rng.uniform(miny, maxy)
                    hill_radius = rng.uniform(0.001, 0.003)
                    hill_height = rng.uniform(10, 40)
                    dist = np.sqrt((X - hill_center_x)**2 + (Y - hill_center_y)**2)
                    relief += hill_height * np.exp(-(dist**2) / (2 * hill_radius**2))
                
                # Ruido mínimo para datos reales
                ...
                st.success(f"✅ Datos SRTM cargados: Elevación promedio = {elev_prom:.1f}m, Pendiente = {slope_prom:.1f}%")
                return X, Y, Z, bounds
            else:
                st.warning("⚠️ No se pudieron obtener datos SRTM. Usando DEM sintético.")
                return generar_dem_sintetico(gdf, 30.0)
                
    except Exception as e:
        st.error(f"❌ Error con SRTM real: {str(e)}")
        return generar_dem_sintetico(gdf, 30.0)
...
def analizar_fertilidad_actual(gdf_dividido, cultivo, datos_satelitales):
    ...
    # Obtener valor base satelital real o simulado
    if datos_satelitales and 'valor_promedio' in datos_satelitales:
        valor_base_satelital = datos_satelitales.get('valor_promedio', 0.6)
        es_dato_real = datos_satelitales.get('fuente', '').find('Simulado') == -1
    else:
        valor_base_satelital = 0.6
        es_dato_real = False
    ...
    for idx, row in gdf_centroids.iterrows():
        ...
        # NDVI: Si es dato real, menos variación
        if es_dato_real:
            # Para datos reales, variación más realista
            ndvi_variacion = patron_espacial * (valor_base_satelital * 0.2)
            ndvi = valor_base_satelital + ndvi_variacion + np.random.normal(0, 0.03)
        else:
            # Para datos simulados, más variación
        ...
        # NDRE: menos variación para datos reales
...
def ejecutar_analisis_completo(gdf, cultivo, n_divisiones, satelite, fecha_inicio, fecha_fin, intervalo_curvas=5.0, resolucion_dem=10.0):
    ...
    try:
        ...
        if satelite == "SENTINEL-2_GEE":
            datos_satelitales = descargar_datos_sentinel2_gee(gdf, fecha_inicio, fecha_fin, "NDVI")
        elif satelite == "LANDSAT-8_GEE":
            datos_satelitales = descargar_datos_landsat8_gee(gdf, fecha_inicio, fecha_fin, "NDVI")
        elif satelite == "SENTINEL-2":
            datos_satelitales = descargar_datos_sentinel2_simulado(gdf, fecha_inicio, fecha_fin, "NDVI")
        elif satelite == "LANDSAT-8":
            datos_satelitales = descargar_datos_landsat8_simulado(gdf, fecha_inicio, fecha_fin, "NDVI")
        else:
        ...
        try:
            # Usar SRTM real si está disponible y seleccionado
            if gee_initialized:
                X, Y, Z, bounds = generar_dem_srtm_real(gdf)
            else:
...
def crear_mapa_fertilidad(gdf_completo, cultivo, datos_satelitales):
    ...
    try:
        ...
        fuente = datos_satelitales.get('fuente', 'Desconocida') if datos_satelitales else 'Simulado'
        es_real = 'REAL' if datos_satelitales and datos_satelitales.get('fuente', '').find('Simulado') == -1 else 'SIMULADO'
        
        ax.set_title(f'{ICONOS_CULTIVOS[cultivo]} FERTILIDAD ACTUAL - {cultivo}\n'
                     f'Fuente: {fuente} | Datos: {es_real}',
...
def generar_reporte_completo(resultados, cultivo, satelite, fecha_inicio, fecha_fin):
    ...
    try:
        ...
        metadatos = [
            ('Generado por', 'Analizador Multi-Cultivo Satellital con Google Earth Engine'),
            ('Versión', '5.0 - Cultivos Extensivos'),
            ('Fecha de generación', datetime.now().strftime("%Y-%m-%d %H:%M:%S")),
            ('Sistema de coordenadas', 'EPSG:4326 (WGS84)'),
            ('Número de zonas', str(len(resultados['gdf_completo']))),
            ('Fuente satelital', resultados.get('datos_satelitales', {}).get('fuente', 'Desconocida')),
            ('NDVI promedio', f"{resultados['gdf_completo']['fert_ndvi'].mean():.3f}"),
            ('Resolución DEM', 'SRTM NASA' if gee_initialized else 'Sintético')
        ]
...
# ===== INTERFAZ PRINCIPAL =====
st.title("ANALIZADOR MULTI-CULTIVO SATELITAL CON GOOGLE EARTH ENGINE")

# Mostrar estado de GEE
if gee_initialized:
    st.success("✅ Google Earth Engine inicializado correctamente")
else:
    st.warning("⚠️ Google Earth Engine no disponible. Usando datos simulados.")
        ...
        try:
            ...
            if gdf is not None:
                ...
                with col2:
                    ...
                    fuente_info = SATELITES_DISPONIBLES.get(satelite_seleccionado, {})
                    fuente_nombre = fuente_info.get('nombre', 'Desconocido')
                    es_real = fuente_info.get('real', False)
                    
                    st.write(f"- Satélite: {fuente_nombre}")
                    st.write(f"- Datos: {'🛰️ REALES' if es_real else '🔬 SIMULADOS'}")
...
if st.session_state.analisis_completado and 'resultados_todos' in st.session_state:
    ...
    # Mostrar información de la fuente de datos
    datos_satelitales = resultados.get('datos_satelitales', {})
    fuente = datos_satelitales.get('fuente', 'Desconocida')
    ndvi_valor = datos_satelitales.get('valor_promedio', 0)
    es_real = datos_satelitales.get('fuente', '').find('Simulado') == -1
    
    st.info(f"**Fuente de datos:** {fuente} | **NDVI promedio:** {ndvi_valor:.3f} | **Tipo:** {'🛰️ REAL' if es_real else '🔬 SIMULADO'}")
    ...
    with tab1:
        ...
        mapa_fert = crear_mapa_fertilidad(resultados['gdf_completo'], cultivo, datos_satelitales)
        if mapa_fert:
...
with col_footer3:
    st.markdown("""
**📞 Soporte:**
- Versión: 6.0 - Google Earth Engine
- Última actualización: Enero 2026
""")

st.markdown(
    '<div style="text-align: center; color: #94a3b8; font-size: 0.9em; margin-top: 2em;">'
    '© 2026 Analizador Multi-Cultivo Satelital con Google Earth Engine. Todos los derechos reservados.'
    '</div>',
    unsafe_allow_html=True
)
[file content end]
