...
# === IMPORTANTE: Instala estas dependencias ===
# pip install owslib rasterio pillow matplotlib geopandas pandas numpy streamlit shapely python-docx requests contextily
...
try:
    ...
    HAS_OWSLIB = False
    st.warning("⚠️ Para datos satelitales reales, instala: pip install owslib rasterio pillow")
...
# ===== CONFIGURACIÓN DE SATÉLITES DISPONIBLES (ACTUALIZADO CON NASA GIBS) =====
SATELITES_DISPONIBLES = {
    'NASA_GIBS': {
        'nombre': 'NASA GIBS (Datos Reales)',
        'resolucion': '250m',
        'revisita': '1-2 días',
        'bandas': ['NDVI', 'RGB'],
        'indices': ['NDVI'],
...
with st.sidebar:
    ...
    satelite_seleccionado = st.selectbox(
        "Satélite:",
        ["NASA_GIBS", "SENTINEL-2", "LANDSAT-8", "DATOS_SIMULADOS"],
        help="NASA GIBS: Datos reales gratuitos de la NASA (250m resolución)"
    )
    
    if satelite_seleccionado == "NASA_GIBS":
        st.info("""
        **🌍 NASA GIBS - Datos Reales:**
        • Resolución: 250m (ideal para parcelas >10 ha)
...
# ===== FUNCIONES NASA GIBS - DATOS SATELITALES REALES =====
def obtener_datos_nasa_gibs(gdf, fecha):
    """
    Obtiene NDVI real de NASA GIBS (MODIS Terra) para la parcela
    Resolución: 250m - Ideal para parcelas grandes (>10 ha)
    """
    try:
        if not HAS_OWSLIB:
            return {
                'success': False,
                'error': 'Instala owslib: pip install owslib rasterio pillow',
                'ndvi_mean': 0.6,
                'source': 'NASA GIBS (no disponible)'
            }
        
        # Configurar WMS de NASA GIBS
        wms_url = "https://gibs.earthdata.nasa.gov/wms/epsg4326/best/wms.cgi"
        wms = WebMapService(wms_url, version='1.1.1')
        
        # Capa MODIS Terra NDVI
        layer_name = 'MODIS_Terra_NDVI'
        ...
        # Formatear fecha (NASA GIBS requiere formato específico)
        fecha_str = fecha.strftime('%Y-%m-%d')
        
        # Calcular tamaño basado en el área
        area = calcular_superficie(gdf)
        if area > 100:  # Más de 100 ha
            size = (800, 600)
        elif area > 10:  # Más de 10 ha
            size = (600, 400)
        else:
            size = (400, 300)
        ...
        try:
            ...
            image = Image.open(io_module.BytesIO(img.read()))
            
            # Convertir a array numpy y calcular NDVI promedio
            img_array = np.array(image.convert('L'))  # Convertir a escala de grises
            
            # Normalizar valores (0-255 a -1 a 1 para NDVI)
            ndvi_data = (img_array / 255.0) * 2 - 1
            
            # Calcular estadísticas solo para píxeles válidos
            valid_pixels = ndvi_data[(ndvi_data >= -1) & (ndvi_data <= 1)]
            
            if len(valid_pixels) > 0:
                ndvi_mean = float(np.nanmean(valid_pixels))
                ndvi_std = float(np.nanstd(valid_pixels))
            else:
                ndvi_mean = 0.6  # Valor por defecto
                ndvi_std = 0.1
            ...
            return {
                ...
                'date': fecha_str,
                'image_data': img.read() if hasattr(img, 'read') else None
            }
            
        except Exception as img_error:
            # Fallback: usar valor simulado basado en el cultivo
            return {
                'success': False,
                'error': str(img_error),
                'ndvi_mean': PARAMETROS_CULTIVOS.get(cultivo, {}).get('NDVI_OPTIMO', 0.6) * 0.8,
                'source': 'NASA GIBS (fallback a simulado)'
            }
        ...
        return {
            ...
            'error': str(e),
            'ndvi_mean': PARAMETROS_CULTIVOS.get(cultivo, {}).get('NDVI_OPTIMO', 0.6) * 0.8,
            'source': 'NASA GIBS (error)'
        }

def obtener_imagen_color_real(gdf, fecha):
    """
    Obtiene imagen RGB real de NASA GIBS
    """
    try:
        if not HAS_OWSLIB:
            return None
        ...
        # Capa de color verdadero (MODIS Terra)
        ...
        ax.imshow(image, extent=[bbox[0], bbox[2], bbox[1], bbox[3]])
        
        # Superponer contorno de la parcela
        gdf_proj = gdf.to_crs('EPSG:4326')
        gdf_proj.boundary.plot(ax=ax, color='red', linewidth=2)
        
        ax.set_title(f'Imagen Satelital Real - NASA GIBS\n{fecha_str}', fontsize=14, fontweight='bold')
        ...
        st.warning(f"⚠️ No se pudo obtener imagen real: {e}")
        return None

# ===== FUNCIONES PARA DATOS SATELITALES SIMULADOS =====
...
def ejecutar_analisis_completo(gdf, cultivo, n_divisiones, satelite, fecha_inicio, fecha_fin, intervalo_curvas=5.0, resolucion_dem=10.0):
    ...
    try:
        ...
        # Obtener datos satelitales REALES si es NASA GIBS
        datos_satelitales = None
        if satelite == "NASA_GIBS":
            # Usar datos reales de NASA
            with st.spinner("🌍 Descargando datos reales de NASA GIBS..."):
                datos_reales = obtener_datos_nasa_gibs(gdf, fecha_fin)
                resultados['datos_reales'] = datos_reales
                
                if datos_reales['success']:
                    datos_satelitales = {
                        'indice': 'NDVI',
                        'valor_promedio': datos_reales['ndvi_mean'],
                        'fuente': datos_reales['source'],
                        'fecha': datos_reales['date'],
                        'resolucion': datos_reales['resolution'],
                        'real': True
                    }
                    st.success(f"✅ Datos reales obtenidos: NDVI = {datos_reales['ndvi_mean']:.3f}")
                else:
                    st.warning(f"⚠️ {datos_reales.get('error', 'Usando datos simulados')}")
        ...
        # Obtener imagen real si es NASA GIBS
        if satelite == "NASA_GIBS":
            with st.spinner("🖼️ Descargando imagen satelital real..."):
                imagen_real = obtener_imagen_color_real(gdf, fecha_fin)
                resultados['imagen_real'] = imagen_real
...
if st.session_state.analisis_completado and 'resultados_todos' in st.session_state:
    ...
    with tab1:
        ...
        if resultados.get('datos_reales') and resultados['datos_reales'].get('success'):
            col_real1, col_real2 = st.columns(2)
            with col_real1:
                st.info(f"**🌍 Datos Reales NASA GIBS:**")
                st.write(f"- NDVI: {resultados['datos_reales']['ndvi_mean']:.3f}")
                st.write(f"- Fuente: {resultados['datos_reales']['source']}")
                st.write(f"- Fecha: {resultados['datos_reales']['date']}")
                st.write(f"- Resolución: {resultados['datos_reales']['resolution']}")
            
            with col_real2:
                if resultados.get('imagen_real'):
                    st.info("**🖼️ Imagen Satelital Real**")
                    st.image(resultados['imagen_real'], use_container_width=True,
                             caption=f"NASA GIBS - {fecha_fin.strftime('%Y-%m-%d')}")
                    
                    # Botón descarga imagen real
                    crear_boton_descarga_png(
                        resultados['imagen_real'],
                        f"imagen_real_{cultivo}_{datetime.now().strftime('%Y%m%d_%H%M')}.png",
                        "📥 Descargar Imagen Satelital PNG"
                    )
...
with col_footer3:
    st.markdown("""
**📞 Soporte:**
- Versión: 6.0 - Con Datos Reales NASA
